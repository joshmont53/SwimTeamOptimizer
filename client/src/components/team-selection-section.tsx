  ...
  return (
    <div className="space-y-8">
      ...
      {teams.length > 0 ? (
        <div className="space-y-8">
          {/* In Progress Teams */}
          {teams.filter(team => team.status !== "selected").length > 0 && (
            <div>
              <h2 className="text-xl font-semibold text-gray-900 mb-4">Existing Teams</h2>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {teams.filter(team => team.status !== "selected").map((team) => (
          ...
          {teams.filter(team => team.status === "selected").length > 0 && (
            <div>
              <h2 className="text-xl font-semibold text-gray-900 mb-4">Final Teams</h2>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {teams.filter(team => team.status === "selected").map((team) => (
                  <Card key={team.id} className="hover:shadow-md transition-shadow cursor-pointer">
                    <CardHeader className="pb-3">
